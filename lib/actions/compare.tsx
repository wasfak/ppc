...
const calculateMetrics = (data: RowData[]) => {
...
export async function uploadAndCompare(formData: FormData) {
  const prevWeekFile = formData.get("prevWeek") as File;
  const currWeekFile = formData.get("currWeek") as File;
  ...
  const formattedComparisonData = comparisonData.map((item) => {
    ...
    const formatSalesImpressions = (value: number) => value.toLocaleString();
  ...
  console.log(formattedComparisonData);
